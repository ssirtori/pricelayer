...
- `node scripts/smoke.mjs` — no formal test suite, but a hand-rolled smoke script (56 asserts) verifies volume/surface-area math (a 10mm cube must be 1 cm³ and 600 mm², origin-independent), the shell-and-infill formula (0.5 mm shell + 20% infill → 0.44 cm³ effective), slicer weight/time detection (slice_info beats gcode per metric; gcode "2h 14m 31s" → 2.242 h), and the wear-cost brackets (`computeWear` €500/4.7 h → €150 maintenance, coefficient 1.30, €0.611 wear) against synthetic `.3mf` files built in memory. It uses the `jsdom` dev-dependency as a `DOMParser` polyfill because three's 3MFLoader needs one under Node. Run it after touching parser/volume/cost logic.
...
- `composables/useWearCost.js` — optional printer wear-and-tear model. `wearBracket(price)` maps purchase price to an accounting useful life and maintenance percentage (≤€300→3.5k h/25%, €301–600→5k/30%, €601–1000→6k/35%, €1001–2000→8k/40%, >€2000→10k/50%; brackets inclusive at the upper bound). `computeWear({ price, timeH, lifespanHours, maintenancePct })` → `maintenanceCost = price × pct`, `maintenanceCoefficient = 1 + maintenanceCost/price`, `totalLifetimeCost = price + maintenanceCost`, `wearCostPerHour = totalLifetimeCost / lifespanHours`, `printWearCost = wearCostPerHour × timeH`; returns `null` when price/time are missing. `lifespanHours`/`maintenancePct` are optional overrides that replace the bracket values (ResultsPanel converts the user's `%` input into a fraction before calling).
...
- Print-cost model: `energy_kWh = averageDrawW/1000 × durationH`, `cost = energy × €/kWh + FilamentWeightKg × €/kg`; user inputs are €/kWh, €/kg (defaults from material), and an optional manual print-duration override that otherwise picks up the slicer's `durationH`. **Optional wear cost** (enabled via a checkbox, `includeWearCost`): `printWearCost` from `composables/useWearCost.js` (needs the user-entered `printerPrice`) is added to the base cost, so `base = energy + filament + wear` and the surcharge applies on top of that base.
...
- **3MF `transform` strings are 12 numbers in column-major order** (linear 3×3 written column-by-column, translation last: indices 9–11). Bambu/Orca write them that way, and three's `parseTransform` reads them as `set(t0,t3,t6,t9, t1,t4,t7,t10, t2,t5,t8,t11, 0,0,0,1)`. `parseTransformMatrix` (use3mfParser.js) and `streamAttrTransform` (useMeshVolume.js) mirror that exactly; the smoke-test component transforms are column-major too. Treating them as row-major silently degenerates the matrix.
- Skipper for huge meshes: `ThreeMFLoader` throws `ERR_STRING_TOO_LONG` (~512 MB JS string cap) on big `.model` parts, so `use3mfParser` falls back to `computeStreamedGeometry` (chunked XML scan via `streamMeshObjects` + `accumulateMeshMeasurement`). Build/component transforms are walked in three's `parentWorld × local` order; validated on `CALZA.3mf` (645 MB mesh, 238 cm³ / 78 678 mm² / 86×91×150 mm). Keep the smoke asserts green when touching either path.
